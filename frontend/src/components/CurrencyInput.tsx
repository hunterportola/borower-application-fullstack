...
const CurrencyInput = React.forwardRef<HTMLInputElement, CurrencyInputProps>(
  ({ 
    className, label, value, error, showError = false, onChange,
    size = 'medium', allowCents = true, ...props 
  }, ref) => {
    ...
    // --- NEW CODE TO FIX CURSOR ---
    useEffect(() => {
        if (isFocused && inputRef.current && displayValue === '$') {
            // When focused and the value is just "$", move cursor to the end.
    ...
    const formatCurrency = (input: string): string => {
      ...
      if (allowCents && digits.includes('.')) {
        ...
        const formattedWhole = wholeNum.toLocaleString()
